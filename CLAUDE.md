...
* **Trading Logic Parameters (all must hold for a BUY):**
  ...
  * **Macro Trend Alignment:** Close Price > 100-day EMA (`MACRO_EMA_PERIOD`).
  * **Trend Strength:** 50-day EMA rising over the last 5 sessions OR 14-day ADX > 20.
  * **Volatility Gate:** ATR(14) / Close ≥ 2%, so a 3 × ATR target is reachable inside the holding window.
  * **Relative Strength:** the stock's 20-day return must beat the KMI-30 index's 20-day return.
  ...
  * **Volume Spike / Near Support (informational by default):** Today's volume ≥ 1.5× the average of the prior 20 sessions, and close no more than 3% above the 50-day EMA or the lowest low of the prior 20 sessions. Requiring both left only 3.7 trades a year in the 3-year backtest, so `STRICT_ENTRY = False` reports them on every signal without blocking it; set it to `True` to require them again.
