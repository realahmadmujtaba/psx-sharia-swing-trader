...
* **Two Setups, Either Triggers a BUY:** momentum and pullback rules are kept apart rather than intersected — mixing them starved the scanner (14 trades in 3 years). `core/signal_engine.SETUPS` holds both; the signal records which one fired (`setup`), and the dashboard and email show it.
  * **Baseline (both setups must clear):** 20-day SMA of volume > 500,000 shares; Close > Rs. 10; Close > 100-day EMA; and the KMI-30 index above its own 50-day EMA (market filter).
  * **Setup A — Breakout:** 14-day ADX > 20, the stock's 20-day return beats the KMI-30's, and today's volume ≥ 1.5× the prior 20-session average. RSI and support are ignored.
  * **Setup B — Pullback:** 14-day RSI < 45 and Close within 3% above the 50-day EMA. ADX and relative strength are ignored.
...
* **Backtest Separation:** `core/signal_engine` takes dataframes and performs no I/O, so `core/backtest.py` replays the identical rules over history. It reports `legacy`, `relaxed` and `strict` variants with brokerage costs, and `docs/backtest.json` feeds the public landing page — including losing periods.
* **SELL Signal = Exit Alert:** PSX retail swing trading is long-only (no shorting), so SELL is not an independent scan — it is the exit for a position a prior BUY alert opened. Once a symbol's BUY alert fires, treat it as an open position until a SELL closes it. Each EOD run, for any open position, fire SELL when the closing price crosses the entry's Stop-Loss or Take-Profit, or when it closes below the `TRAIL_EMA_PERIOD` (20) EMA — a trailing exit that lets winners run instead of cutting them on a fixed calendar day. There is no time-based exit. Do not scan a symbol for a new BUY while it already has an open position.
